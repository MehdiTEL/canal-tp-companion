      ...
      <div className="max-w-sm mx-auto mt-20 animate-slide-up">
        <div className="bg-surface-card rounded-xl shadow-elevated p-6 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display font-bold text-text-on-light">Espace formateur</h2>
          </div>
          <form onSubmit={handleAuth} className="space-y-3">
            <input type="password" value={code} onChange={(e) => setCode(e.target.value)}
              placeholder="Code d'acces" aria-label="Code d'acces formateur"
          ...
          {code && code !== FORMATEUR_CODE && code.length >= 4 && (
            <p className="text-[13px] text-red-500 font-body">Code incorrect</p>
          )}
        </div>
      </div>
    );
  ...
  if (!supabase) {
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <h1 className="text-xl font-display font-bold text-text-on-light">Espace formateur</h1>
        <div className="bg-warning-bg border border-warning/20 rounded-lg p-4 text-[14px] text-text-body font-body">
          Supabase n'est pas configure. Configurez les variables VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY dans le fichier .env.
        </div>
      </div>
    );
  }
  ...
  if (selectedParticipant) {
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        {subs.length === 0 ? (
          <p className="text-[14px] text-text-muted font-body">Aucune soumission.</p>
        ) : (
              ...
              <div key={s.id} className="bg-surface-card rounded-lg shadow-card p-4 space-y-2">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-text-muted font-body">
                    {s.self_rating && <span>Note : {s.self_rating}/5</span>}
                    {s.hints_used > 0 && <span>Astuces : {s.hints_used}</span>}
                    {s.completed && <span className="text-success font-semibold">Termine</span>}
                  </div>
                </div>
                {s.prompt_text && (
                  <div>
                    <p className="text-[11px] font-semibold text-text-muted mb-1">Prompt :</p>
                    <p className="text-[13px] text-text-body whitespace-pre-wrap bg-surface-elevated rounded-md p-3 font-body">{s.prompt_text}</p>
                  </div>
                )}
                {s.result_text && (
                  <div>
                    <p className="text-[11px] font-semibold text-text-muted mb-1">Resultat :</p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-text-muted font-body mt-0.5">
            {participants.length} participant{participants.length !== 1 ? 's' : ''} / {submissions.length} soumission{submissions.length !== 1 ? 's' : ''}
          </p>
        </div>
          ...
          <table className="w-full text-[13px] font-body">
            <thead>
              <tr className="bg-surface-elevated border-b border-border-subtle">
                <th className="text-left px-4 py-2.5 font-semibold text-text-body">Metier</th>
                <th className="text-center px-4 py-2.5 font-semibold text-text-body">Sprints</th>
                <th className="text-center px-4 py-2.5 font-semibold text-text-body">Exercices</th>
                <th className="text-center px-4 py-2.5 font-semibold text-text-body">Astuces</th>
                <th className="text-center px-4 py-2.5 font-semibold text-text-body">Actions</th>
              </tr>
            </thead>
            <tbody>
              ...
              {filteredParticipants.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-text-muted">Aucun participant.</td></tr>
              )}
            </tbody>
          </table>
        </div>
